...
export function ServerSelector({ compact = false }: { compact?: boolean }) {
  ...
  async function handleChange(event: React.ChangeEvent<HTMLSelectElement>) {
    ...
    setError("");
    try {
      await selectServer({ serverId: value, serverName: value === "demo-server" ? "My Discord server" : value });
      ...
      <div className="mb-6">
        ...
        <div className="relative">
          ...
          >
            ...
            {user?.selectedServerId && <option value={user.selectedServerId}>{user.selectedServerName ?? "Connected server"}</option>}
          </select>
